...
create policy "Users can view own landlord record"
  on landlords for select using (auth.uid() = user_id);

create policy "Users can insert own landlord record"
  on landlords for insert with check (auth.uid() = user_id);

create policy "Users can update own landlord record"
  on landlords for update using (auth.uid() = user_id);

-- Properties
create policy "Landlords can CRUD own properties"
...
-- Rent payments
create policy "Landlords can CRUD own payments"
  on rent_payments for all using (
    landlord_id in (select id from landlords where user_id = auth.uid())
  );

-- Expenses
create policy "Landlords can CRUD own expenses"
  on expenses for all using (
    landlord_id in (select id from landlords where user_id = auth.uid())
  );

-- Communications
create policy "Landlords can CRUD own communications"
  on communications for all using (
    landlord_id in (select id from landlords where user_id = auth.uid())
  );

-- Maintenance
create policy "Landlords can CRUD own maintenance"
...
create index idx_rent_payments_landlord_id on rent_payments(landlord_id);
...
create index idx_communications_landlord_id on communications(landlord_id);
...
-- Storage RLS
-- create policy "Landlords can upload their own receipts"
--   on storage.objects for insert with check (bucket_id = 'receipts' AND auth.uid()::text = (storage.foldername(name))[1]);
-- create policy "Landlords can view their own receipts"
